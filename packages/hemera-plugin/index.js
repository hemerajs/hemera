...
function plugin (fn, version) {
  ...
  if (version) {
    if (typeof version !== 'string') {
      throw new TypeError(`hemera-plugin expects a version string as second parameter, instead got '${typeof version}'`)
    }

    const hemeraVersion = require('nats-hemera/package.json').version
    if (!semver.satisfies(hemeraVersion, version)) {
      throw new Error(`hemera-plugin - expected '${version}' nats-hemera version, '${hemeraVersion}' is installed`)
    }
  }
