...
class Hemera extends EventEmitter {
  ...
  * @param {any} replyTo
  * @param {any} result
  * @param {any} shouldCrash
  * @returns
  *
  * @memberOf Hemera
  */
  reply(replyTo, result, shouldCrash) {
    ...
    const msg = Util.stringifyJSON(this.message)

    if (shouldCrash) {

      //Send error back to callee
      return this.send(replyTo, msg, () => {
  ...
  subscribe(topic) {
    ...
    this.transport.subscribe(topic, {
      ...
      AddContext.OnPreProcessing(ctx, result.value)
      ...
      if (result.error) {
        ...
        return ctx.reply(replyTo, error)
      }
      ...
      if (actMeta) {
        ...
        try {
          ...
          //Validate payload
          ctx._payloadValidator.validate(actMeta.schema, result.value.pattern, (err, value) => {

            if (err) {

              let error = new Errors.PayloadValidationError(Constants.PAYLOAD_VALIDATION_ERROR, {
                pattern
              }).causedBy(err)
            ...
            action(value, (err, resp) => {
              ...
              if (err) {
                ...
                let error = new Errors.BusinessError(Constants.IMPLEMENTATION_ERROR, {
                  pattern
                }).causedBy(err)
  ...
  act(pattern, cb) {
    ...
    //Request to topic
    let sid = this.sendRequest(pattern.topic, Util.stringifyJSON(ctx.message), (response) => {
      ...
      try {
        ...
        ActContext.onPostRequest(ctx, msg.value, cleanPattern)
