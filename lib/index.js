...
class Hemera extends EventEmitter {
        ...
        if (actMeta) {
          ...
          try {
            ...
            paramcheck.validate(result, (err) => {
              ...
              ctx.requestId$ = result.request$.id
  ...
  act(pattern, cb) {
      ...
      ctx.context$ = pattern.context$ || this.context$
      ...
      ctx.requestId$ = pattern.requestId$ || this.createRequestId()
      ...
      let sid = this.sendRequest(pattern.topic, this.stringifyJSON(msg), (response) => {
        ...
        try {
          ...
          ctx.request$ = msg.value.request$

          //Emit event
          this.emit('inbound', msg.value)

          this.log().info(pattern, `ACT_INBOUND - ID:${msg.value.request$.id}`)
